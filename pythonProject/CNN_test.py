...
with torch.no_grad():
    ...
    accuracy1 = (accuracy1+0.1)*100
print(f'CNN_Training Accuracy: {accuracy1}','%')

# 测试单独的测试文件
test_file = "E:/tyy_confuse/Pcaps/LSTM_test/CHAT_mimicry_training_add.json"
with open(test_file, 'r') as f:
    test_data = json.load(f)

# 处理测试文件数据
X_test = np.array([[sample['length'], sample['time_difference']] for sample in test_data], dtype=np.float32)
y_test = np.array([1 if sample['label'] == 'CHAT' else 0 for sample in test_data])
# y_test = np.array([1 if sample['label'] == 'Browsing' else
#                    2 if sample['label'] == 'CHAT' else
#                    3 if sample['label'] == 'P2P' else
#                    4 if sample['label'] == 'VOIP' else
#                    0 for sample in test_data])
...
with torch.no_grad():
    ...
    browsing_prob = (browsing_prob)*100
    chat_prob  = (chat_prob )*100
    p2p_prob = (p2p_prob ) * 100
    voip_prob = (voip_prob ) * 100
print('----------------------------------------')
print('--The result of the CNN classification--')
